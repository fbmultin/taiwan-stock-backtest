...
// 「我的常用標的」用瀏覽器 localStorage 記住使用者自己常用的標的組合，一鍵套用即可取代目前輸入,不用每次都重打;使用者也可隨時按「設為常用」更新這組合。
...
// 交易日數不足時回傳 null,卡片上改顯示「資料不足」而非硬涨一個不可靠的數字。
...
// 不同資料源(FinMind、Yahoo)是否已經把歷史股價回溷調整過並不一致,
